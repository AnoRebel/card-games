...
export function sweepTo(targets: string | Element | Element[], dx = 0, dy = -28) {
  if (reduced()) return;
  animate(targets, {
...
export function loseShake(target: string | Element): void {
  ...
  animate(target, {
    ...
    // Spread the desaturation pulse in only when rich: anime.js rejects an
    // explicit `filter: undefined`, so the key has to be absent entirely.
    ...(rich() ? { filter: ["saturate(1)", "saturate(0.6)", "saturate(1)"] } : {}),
