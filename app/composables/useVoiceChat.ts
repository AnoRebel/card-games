...
export function useVoiceChat(opts: UseVoiceChatOptions) {
  ...
  const inVoice = computed(
    () => state.value === "connecting" || state.value === "connected",
  );
  ...
  async function makeOffer(conn: PeerConn) {
    try {
      ...
      sendWs({
        ...
        to: conn.peerId,
        from: myId,
        data: { sdp: conn.pc.localDescription },
      });
    } catch {
  ...
  function onWsMessage(raw: string) {
    ...
    const t = msg.t;
    if (t === "peers") {
      const list = Array.isArray(msg.peers)
        ? (msg.peers as { peerId: string; name: string }[])
        : [];
      for (const p of list) {
  ...
  async function join() {
    ...
    socket.addEventListener("open", () => {
      sendWs({
        t: "join",
        roomId: toValue(opts.roomId),
        peerId: myId,
        name: toValue(opts.name),
      });
    });
