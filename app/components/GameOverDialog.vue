...
const rows = computed(() => {
  ...
  return props.players
    .map((p) => ({
      ...
      name: p.name,
      score:
        (useVp.value ? s.victoryBySeat?.[p.seat] : undefined) ?? s.bySeat[p.seat] ?? 0,
...
const youWon = computed(
  () =>
    !!props.scores &&
    props.viewerSeat != null &&
    props.scores.winners.includes(props.viewerSeat),
);
      ...
      <div v-else class="space-y-4">
        ...
        <ol class="space-y-1">
          <li
            ...
            :style="{
              background: row.won
                ? 'color-mix(in oklch, var(--cg-accent) 16%, transparent)'
                : 'var(--cg-surface)',
              border: row.won
          ...
          >
            ...
            <span
              ...
              :style="{ color: row.won ? 'var(--cg-accent)' : 'var(--cg-text)' }"
            >
              {{ row.score }}
            </span>
          </li>
